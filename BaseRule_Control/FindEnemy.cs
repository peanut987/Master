...
public class FindEnemy : MonoBehaviour
{
    ...
    [System.Serializable]
    public class K_Means_Group_Info
    {
        public Vector3 Average = Vector3.zero;//组的中心点
        public Vector3 Last_Frame_Average = Vector3.zero;//上一帧组的中心点
        public List<int> Group = new List<int>();//各组坦克 存的是敌方坦克编号 
        public float Group_avg_dist = 0;//各组距离误差 
        public float Group_max_dist = 0;
        public float Average_angle = 0;//从本组中心点到另一组组中心点的角度 x-z二维平面 x-y替换成x-z
        public float dist_Average_to_central = 0;//本组中心点到交界带的距离
        public Vector3 Central_SpecialPoint = Vector3.zero;//交界带特征点
        public void Clear()
        {
            this.Average = Vector3.zero;
            this.Group.Clear();
            this.Group_avg_dist = 0;
            this.Group_max_dist = 0;
            this.dist_Average_to_central = 0;
        }
    }

    [System.Serializable]

    public class Move_In_Group_Info
    {
        public int team_num = 0;//攻方侧边个数
        public List<int> Group = new List<int>();//各组坦克 存的是敌方坦克编号 //这四个不能直接拖到函数外public显示 需复制
        public int leader_num = 0;//领队坦克编号
